...
# ── Etiquetas de delivery_status (valores reales del dataset) ──
STATUS_ON_TIME = "Delivered"   # entregado = a tiempo
...
# ── Helpers de comparacion vs periodo anterior ───────────
...
KPI_SQL = """
    SELECT
        ...
        ROUND(AVG(CASE WHEN delivery_status = '{on_time}' THEN 1.0 ELSE 0.0 END)::numeric * 100, 1) AS pct_on_time,
        ROUND(AVG(CASE WHEN delivery_status = '{delayed}' THEN 1.0 ELSE 0.0 END)::numeric * 100, 1) AS pct_delayed
    FROM analytics.fact_orders {where}
""".format(on_time=STATUS_ON_TIME, delayed=STATUS_DELAYED, where=where_log)

kpis_log  = query(KPI_SQL, params)
prev_kpis = query(KPI_SQL, previous_period_params(filters, params))
...
c1.markdown(kpi_card("Entregas a Tiempo",  f"{pct_on_time}%",  pct_delta(kpis_log, prev_kpis, "pct_on_time"), ICON_CHECK, "% de pedidos entregados (estado Delivered) en el período. Subir es bueno.", invert=False), unsafe_allow_html=True)
c2.markdown(kpi_card("Tiempo Prom. Envío", f"{avg_ship} días", pct_delta(kpis_log, prev_kpis, "avg_ship"),    ICON_TRUCK, "Días promedio desde la compra hasta la entrega. Subir es malo.", invert=True), unsafe_allow_html=True)
c3.markdown(kpi_card("Pedidos Demorados",  f"{pct_delayed}%",  pct_delta(kpis_log, prev_kpis, "pct_delayed"), ICON_WARN,  "% de pedidos con estado Delayed. Subir es malo.", invert=True), unsafe_allow_html=True)
c4.markdown(kpi_card("Tasa Devolución",    f"{ret_rate}%",     pct_delta(kpis_log, prev_kpis, "ret_rate"),    ICON_RET,   "% de órdenes devueltas (devueltas / total). Subir es malo.", invert=True), unsafe_allow_html=True)
...
with col1:
    with st.container(key="chartcard_log_est"):
        df_est = query(f"""
        ...
        fig1.update_layout(**LAYOUT, margin=dict(t=10, b=20, l=10, r=45))
        fig1.update_xaxes(gridcolor="#EEF2F7")
        st.plotly_chart(fig1, use_container_width=True, config={"displayModeBar": False})

with col2:
    with st.container(key="chartcard_log_city"):
        df_city = query(f"""
            SELECT location,
        ...
        fig2.update_layout(**LAYOUT, margin=dict(t=10, b=20, l=10, r=65))
...
with col3:
    with st.container(key="chartcard_log_retcat"):
        df_ret_cat = query(f"""
            SELECT category,
                   ROUND(AVG(CASE WHEN is_returned THEN 1.0 ELSE 0.0 END) * 100, 1) AS tasa_dev,
                   COUNT(*) AS ordenes
        ...
        fig3.update_layout(**LAYOUT, margin=dict(t=10, b=20, l=10, r=45))
        fig3.update_xaxes(gridcolor="#EEF2F7")
        st.plotly_chart(fig3, use_container_width=True, config={"displayModeBar": False})

with col4:
    with st.container(key="chartcard_log_retcity"):
        df_ret_city = query(f"""
            SELECT location,
                   ROUND(AVG(CASE WHEN is_returned THEN 1.0 ELSE 0.0 END) * 100, 1) AS tasa_dev,
                   COUNT(*) AS ordenes
        ...
        fig4.update_layout(**LAYOUT, margin=dict(t=10, b=20, l=10, r=45))
...
with col5:
    with st.container(key="chartcard_log_trend"):
        df_trend = query(f"""
        ...
        fig5.update_layout(**LAYOUT, margin=dict(t=30, b=40, l=10, r=10),
                           xaxis=dict(showgrid=False, tickangle=-30))
        fig5.update_yaxes(gridcolor="#EEF2F7")
        st.plotly_chart(fig5, use_container_width=True, config={"displayModeBar": False})

with col6:
    with st.container(key="chartcard_log_dvr"):
        df_dvr = query(f"""
        ...
        """, params)
        chart_header("Días de Envío vs Tasa de Devolución", df_dvr, "dias_envio_vs_devolucion.csv", "exp_log_dvr",
                     ratio=(18, 1),
                     info="Tasa de devolución según los días que tardó el envío. Ayuda a ver si los envíos más lentos generan más devoluciones.")
        fig6 = go.Figure()
        ...
        fig6.update_layout(**LAYOUT, margin=dict(t=10, b=40, l=10, r=10),
...
with st.container(key="chartcard_log_map"):
    df_map = query(f"""
        SELECT location,
               ROUND(AVG(shipping_time_days)::numeric, 0) AS dias_prom,
               ROUND(AVG(CASE WHEN is_returned THEN 1.0 ELSE 0.0 END)*100,1) AS tasa_dev,
        ...
        GROUP BY location
    """, params)
    chart_header("Distribución Geográfica — Ciudades", df_map, "distribucion_geografica.csv", "exp_log_map",
                 ratio=(24, 1),
                 info="Mapa de ciudades: el tamaño del punto es el volumen de órdenes y el color la tasa de devolución (más rojo = más devoluciones).")
    ...
    fig_map.update_layout(
        ...
        coloraxis_colorbar=dict(title="Dev %", thickness=12, len=0.6, tickfont=dict(size=10)),
        hoverlabel=dict(bgcolor="white", bordercolor="#E4E9F0", font=dict(family=FONT, size=12)),
    )
...
with st.container(key="chartcard_log_pay"):
    df_pay_ret = query(f"""
    ...
    fig7.update_layout(**dict(LAYOUT, height=320), margin=dict(t=30, b=40, l=10, r=10))
